...
`list` reports `mode: inherit_all` when `enabled_builtin_skill_ids` is `null`;
otherwise it reports `mode: exact`. `enable` and `disable` atomically change the
agent policy, while `reset` restores inheritance so future built-ins are enabled
automatically. Enabling an already inherited skill is a no-op and does not
freeze the current inventory into an exact list.
